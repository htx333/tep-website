  ...
  return (
    <div className="rounded-2xl bg-[#ededf0] px-6 py-16 sm:px-10">
      <div className="mx-auto grid max-w-5xl grid-cols-2 items-center justify-items-center gap-x-10 gap-y-16 sm:grid-cols-3 lg:grid-cols-4">
        {partners.map((p) => (
          <div key={p.name} className="flex h-14 w-full items-center justify-center px-2">
            {p.logo ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img
                src={p.logo}
                alt={p.name}
                className="max-h-10 w-auto max-w-full object-contain transition-transform duration-300 hover:scale-105"
              />
            ) : (
